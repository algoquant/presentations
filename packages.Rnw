% Define knitr options
% !Rnw weave = knitr
% Set global chunk options
<<knitr_setup,include=FALSE,cache=FALSE>>=
library(knitr)
opts_chunk$set(comment=NA, highlight=FALSE, size='scriptsize', fig.width=4, fig.height=4)
options(width=60, dev='pdf')
thm <- knit_theme$get("acid")
knit_theme$set(thm)
@


% Define document options
\documentclass[10pt]{beamer}
\mode<presentation>
\usetheme{AnnArbor}
% \usecolortheme{whale}
% Uncover everything in a step-wise fashion
% \beamerdefaultoverlayspecification{<+->}
% amsmath package for math symbols
% \usepackage{amsmath}
\usepackage[latin1]{inputenc}
\usepackage{hyperref}
\usepackage{fancybox}
\usepackage{url}
\usepackage[backend=bibtex,style=alphabetic]{biblatex} % bibstyle=numeric
% \bibliographystyle{amsalpha} % doesn't work
\addbibresource{FRE6811_bib.bib}
% \addbibresource[location=remote]{http://www.citeulike.org/user/jerzyp}
\renewcommand\bibfont{\footnotesize}
\renewcommand{\pgfuseimage}[1]{\scalebox{0.75}{\includegraphics{#1}}} % scale bib icons
\setbeamertemplate{bibliography item}[text] % set bib icons
% \setbeamertemplate{bibliography item}{} % remove bib icons

% \usepackage{enumerate}
% \let\emph\textbf
% \let\alert\textbf
% Define colors for hyperlinks
\definecolor{links}{HTML}{2A1B81}
\hypersetup{colorlinks=true,linkcolor=,urlcolor=links}
% Make url text footnotesize
\renewcommand\UrlFont{\footnotesize}
% Make institute text italic and small
\setbeamerfont{institute}{size=\small,shape=\itshape,bg=red,fg=red}
\setbeamerfont{date}{size=\small}
\setbeamerfont{block title}{size=\normalsize} % shape=\itshape
\setbeamerfont{block body}{size=\footnotesize}


% Title page setup
\title[FRE6811 Lecture \#1]{FRE6811 Financial Software Laboratory}
\subtitle{Lecture \#1, Spring 2014}
% \subject{Getting Started With R}
\institute[NYU Polytechnic]{NYU Polytechnic School of Engineering}
\titlegraphic{\includegraphics[scale=0.8]{engineering_long_color}}
\author{Jerzy Pawlowski}
% \email{jp3900@poly.edu}
\date{January 27, 2014}
% \date{\today}
% \pgfdeclareimage[height=0.5cm]{university-logo}{engineering_long_white}
% \logo{\pgfuseimage{engineering_long_white}}


%%%%%%%%%%%%%%%
\begin{document}


%%%%%%%%%%%%%%%
\maketitle


%%%%%%%%%%%%%%%
\section{Basic Operations in $R$}


%%%%%%%%%%%%%%%
\subsection{Assignment Operators}
\begin{frame}[fragile,t]{\subsecname}
\begin{block}{}
  \begin{columns}[T]
    \column{0.6\textwidth}
      The standard assignment operator in $R$ is \verb|"<-"|,\\
      The \verb|"="| operator is used for named arguments in function calls,\\
      Both \verb|"<-"| and \verb|"="| are valid assignment operators in $R$,\\
      But they differ in \emph{scope} and  \emph{precedence} (\verb|"<-"| has higher precedence than \verb|"="|),\\
    \column{0.4\textwidth}
      \vspace{-1em}
      <<prompt=TRUE>>=
        n.var1 <- 3 - 1
        n.var1
        n.var1 = 3 - 1
        n.var1
      @
  \end{columns}
\end{block}
\pause

\begin{block}{\color{red}{Rule of Thumb}}
Use \verb|"<-"| in $R$ scripts,\\
Use \verb|"="| in function calls.\\
\end{block}

\end{frame}


%%%%%%%%%%%%%%%
\subsection{The $R$ Workspace}
\begin{frame}[fragile,t]{\subsecname}

\begin{block}{}
  \begin{columns}[T]
    \column{0.3\textwidth}
      Get default (working) directory
    \column{0.7\textwidth}
      \vspace{-1em}
      <<prompt=TRUE>>=
        getwd()
      @
  \end{columns}
\end{block}
\pause

\begin{block}{}
  \begin{columns}[T]
    \column{0.3\textwidth}
      Get help on a function
    \column{0.7\textwidth}
      \vspace{-1em}
% tidy=FALSE prevents translation of '?getwd' into '`?`(getwd)'
      <<tidy=FALSE,eval=FALSE,out.width='0.3\textwidth'>>=
        ?getwd or help(getwd)
      @
  \end{columns}
\end{block}

\end{frame}


%%%%%%%%%%%%%%%
\section{Data Structures}


%%%%%%%%%%%%%%%
\subsection{Vectors}
\begin{frame}[fragile,t]{\subsecname}

\begin{block}{}
  \begin{columns}[T]
    \column{0.4\textwidth}
      The function \verb|c()| combines values into a vector or a list,\\
      Vectors can be multiplied and squared,\\
    \column{0.6\textwidth}
      \vspace{-1em}
      <<prompt=TRUE>>=
        v.var <- c(pi, exp(1), sqrt(2))
        v.var[2] # get second element
        2*v.var # multiply all elements by 2
        v.var^2 # square all elements
      @
  \end{columns}
\end{block}

\begin{block}{}
  \begin{columns}[T]
    \column{0.4\textwidth}
      Vector elements can be assigned $names$,\\
    \column{0.6\textwidth}
      \vspace{-1em}
      <<prompt=TRUE>>=
        v.var <- c(pi=pi, e=exp(1), sqrt2=sqrt(2))
        v.var
        names(v.var)
        names(v.var) <- c("pie","eulery","sqrt2y")
        v.var
      @
  \end{columns}
\end{block}

\end{frame}


%%%%%%%%%%%%%%%
\subsection{Variable Class and Type}
\begin{frame}[fragile,t]{\subsecname}

\begin{block}{}
  \begin{columns}[T]
    \column{0.4\textwidth}
      The function \verb|c()| combines values into a vector or a list,\\
      Vectors can be multiplied and squared,\\
    \column{0.6\textwidth}
      \vspace{-1em}
      <<prompt=TRUE>>=
        v.var <- c(pi, exp(1), sqrt(2))
        v.var[2] # get second element
        2*v.var # multiply all elements by 2
        v.var^2 # square all elements
      @
  \end{columns}
\end{block}

\begin{block}{}
  \begin{columns}[T]
    \column{0.4\textwidth}
      Vector elements can be assigned $names$,\\
    \column{0.6\textwidth}
      \vspace{-1em}
      <<prompt=TRUE>>=
        v.var <- c(pi=pi, e=exp(1), sqrt2=sqrt(2))
        v.var
        names(v.var)
        names(v.var) <- c("pie","eulery","sqrt2y")
        v.var
      @
  \end{columns}
\end{block}

\end{frame}


%%%%%%%%%%%%%%%
\subsection{Matrices}
\begin{frame}[fragile,t]{\subsecname}
\begin{block}{}
  \begin{columns}[T]
    \column{0.4\textwidth}
    Subsetting Vectors, Matrices and Data Frames
      The function \verb|c()| combines values into a vector or a list,\\
      The \verb|"="| operator is used for named arguments in function calls,\\
      Both \verb|"<-"| and \verb|"="| are valid assignment operators in $R$,\\
      But they differ in \emph{scope} and  \emph{precedence} (\verb|"<-"| has higher precedence than \verb|"="|),\\
    \column{0.6\textwidth}
      \vspace{-1em}
      <<prompt=TRUE>>=
        v.var <- c(pi, exp(1), sqrt(2))
        v.var
        2*v.var
        v.var^2
        names(v.var)
        v.var <- c(pi=pi, e=exp(1), sqrt2=sqrt(2))
        v.var
        names(v.var) <- c("pie","eulery","sqrt2y")
        v.var
        class(v.var)
        typeof(v.var)
      @
  \end{columns}
\end{block}
\pause

\begin{block}{\color{red}{Rule of Thumb}}
Use \verb|"<-"| in $R$ scripts,\\
Use \verb|"="| in function calls.\\
\end{block}

\end{frame}


%%%%%%%%%%%%%%%
\subsection{Lists}
\begin{frame}[fragile,t]{\subsecname}
\begin{block}{}
  \begin{columns}[T]
    \column{0.4\textwidth}
    Subsetting Vectors, Matrices and Data Frames
      The function \verb|c()| combine values into a vector or a list,\\
      The \verb|"="| operator is used for named arguments in function calls,\\
      Both \verb|"<-"| and \verb|"="| are valid assignment operators in $R$,\\
      But they differ in \emph{scope} and  \emph{precedence} (\verb|"<-"| has higher precedence than \verb|"="|),\\
    \column{0.6\textwidth}
      \vspace{-1em}
      <<prompt=TRUE>>=
        v.var <- c(pi, exp(1), sqrt(2))
        v.var
        2*v.var
        v.var^2
        names(v.var)
        v.var <- c(pi=pi, e=exp(1), sqrt2=sqrt(2))
        v.var
        names(v.var) <- c("pie","eulery","sqrt2y")
        v.var
        class(v.var)
        typeof(v.var)
      @
  \end{columns}
\end{block}
\pause

\begin{block}{\color{red}{Rule of Thumb}}
Use \verb|"<-"| in $R$ scripts,\\
Use \verb|"="| in function calls.\\
\end{block}

\end{frame}


%%%%%%%%%%%%%%%
\section{$R$ Packages}


%%%%%%%%%%%%%%%
\subsection{Loading Packages}
\begin{frame}[fragile,t]{\subsecname}

\begin{block}{Loading Packages}
$R$ can run external libraries of functions called packages,\\
Packages need to be loaded before they can be used,\\
  \begin{columns}[T]
    \column{0.5\textwidth}
      There are two ways of loading package 'MASS'
    \column{0.5\textwidth}
      \vspace{-1em}
      <<tidy=FALSE,eval=FALSE>>=
        library("MASS") or require("MASS")
      @
  \end{columns}
\end{block}
%\vskip1ex
\pause

\begin{block}{Difference between library() and require()}
\verb|library()| halts execution if the package can't be loaded,\\
\verb|require()| returns a logical value, TRUE if the package is loaded, FALSE if it isn't.\\
\end{block}
\pause

\begin{block}{\color{red}{Rule of Thumb}}
Use \verb|library()| in script files that might be sourced,\\
Use \verb|require()| inside other functions.\\
\end{block}
\pause

\begin{block}{Installing Packages}
Some packages like $MASS$ are part of the $R$ distribution,\\
But most packages need to be $installed$ before they can be loaded,\\
\end{block}

\end{frame}


%%%%%%%%%%%%%%%
\subsection{Installing Packages}
\begin{frame}[fragile,t]{\subsecname}

\begin{block}{Installing Packages from $CRAN$}
Most of the widely used packages are available on $CRAN$,\\
For example package $AER$:\\
\hskip1em\url{http://cran.us.r-project.org/web/packages/AER/}\\
  \begin{columns}[T]
    \column{0.5\textwidth}
      Install and load package $AER$ in $R$ Session:
    \column{0.5\textwidth}
      \vspace{-1em}
      <<tidy=FALSE,eval=FALSE,prompt=TRUE>>=
        install.packages("AER")
        library("AER")
      @
  \end{columns}
\end{block}
\pause

\begin{block}{Installing packages in $RStudio$}
Go to \alert{Tools} and then \alert{Install packages}\\
Then enter package name, for example $AER$
\end{block}
\pause

\begin{block}{Packages on $R-Forge$}
Many other packages are available on $R-Forge$:\\
\hskip1em\url{https://r-forge.r-project.org/}\\
For example packages which are part of the $ReturnAnalytics$ project,\\
\hskip1em\url{https://r-forge.r-project.org/projects/returnanalytics/}\\
\hskip1em\url{https://r-forge.r-project.org/R/?group_id=579}\\
  \begin{columns}[T]
    \column{0.3\textwidth}
      Install package $PerformanceAnalytics$
    \column{0.7\textwidth}
      \vspace{-1em}
      <<tidy=FALSE,eval=FALSE,prompt=TRUE>>=
        install.packages("PerformanceAnalytics", 
                         repos="http://R-Forge.R-project.org")
      @
  \end{columns}
\end{block}

\end{frame}


%%%%%%%%%%%%%%%
\subsection{$CRAN$ Package Views}
\begin{frame}[fragile,t]{\subsecname}

\begin{columns}[T]
  \column{0.4\textwidth}
    \begin{block}{}
      $CRAN$ view for package $AER$:\\
      \hskip1em\url{http://cran.us.r-project.org/web/packages/AER/}\\
    \end{block}
    \begin{block}{}
      Note:
      \begin{itemize}[]
        \item Authors,
        \item Version number,
        \item Reference manual,
        \item Vignettes,
        \item Dependencies on other packages.
      \end{itemize}
    \end{block}
  \column{0.6\textwidth}
    \vspace{-1em}
    \includegraphics[height=1.0\textwidth]{CRAN_Package.png}
\end{columns}

\end{frame}


%%%%%%%%%%%%%%%
\subsection{$CRAN$ Task Views}
\begin{frame}[fragile,t]{\subsecname}

\begin{columns}[T]
  \column{0.4\textwidth}
    \begin{block}{}
      $CRAN$ Finance Task View\\
      \hskip1em\url{http://cran.us.r-project.org/}\\
    \end{block}
    \begin{block}{}
      Note:
      \begin{itemize}[]
        \item Maintainer,
        \item Topics,
        \item List of packages.
      \end{itemize}
    \end{block}
  \column{0.6\textwidth}
    \vspace{-1em}
    \includegraphics[height=1.0\textwidth]{CRAN_Views.png}
\end{columns}

\end{frame}


%%%%%%%%%%%%%%%
\subsection{Exploring Packages}
\begin{frame}[fragile,t]{\subsecname}

\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      Install package $AER$
    \column{0.5\textwidth}
      \vspace{-1em}
      <<tidy=FALSE,eval=FALSE,prompt=TRUE>>=
        packageDescription("MASS") # short description
        help(package="MASS") # full documentation
        ls("package:MASS") # list functions
        search() # show all loaded packages
        data("Journals", package = "AER")
        detach("package:AER") # remove package from list
      @
  \end{columns}
\end{block}

\end{frame}


%%%%%%%%%%%%%%%
\subsection{References}
\begin{frame}[fragile,t]{\subsecname}

There will be no required textbook, but a recommended textbook is:\\
{\small{\fullcite{rupperttexbook}}}\\

$RStudio$:\\
{\small{\fullcite{website:rstudioide}}}\\

An R resource:\\
{\small{\fullcite{website:cran}}}\\

Google Style Guide for R:\\
{\small{\fullcite{website:googlestyler}}}\\

\end{frame}


%%%%%%%%%%%%%%%
\subsection{Bibliography}
\begin{frame}[fragile,t,allowframebreaks]{\subsecname}

\nocite{*} % print all bib file entries
{\tiny{\printbibliography}}

% doesn't work as expected - prints only books
% \printbibliography[type=book,title={Books}] % print books
% \printbibliography[type=online,title={Websites}] % print Websites

\end{frame}


%%%%%%%%%%%%%%%
\subsection{Creative Commons Attribution-ShareAlike License}
\begin{frame}[t]{\subsecname}
\small{
\begin{columns}[T]
  \column[T]{0.5\textwidth}
    You are free to:
    \vspace{0.6em}
    \begin{itemize}[<+->]
      \item Share - copy and redistribute the material in any medium or format.
      \item Adapt - remix, transform, and build upon the material for any purpose, even commercially.
    \end{itemize}
    \pause
  \column[T]{0.5\textwidth}
    Under the following terms:
    \begin{itemize}[<+->]
      \item Attribution - You must give appropriate credit, provide a link to the license, and indicate if changes were made. You may do so in any reasonable manner, but not in any way that suggests the licensor endorses you or your use.
      \item ShareAlike - If you remix, transform, or build upon the material, you must distribute your contributions under the same license as the original.
    \end{itemize}
\end{columns}
}
\end{frame}


\end{document}

