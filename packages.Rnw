...
Some packages like \verb|MASS| are part of the $R$ distribution,\\
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.3\textwidth}
      Install package \verb|AER|
    \column{0.7\textwidth}
      \vspace{-1em}
      <<tidy=FALSE,eval=FALSE,prompt=TRUE>>=
        search()  # show all loaded packages
        packageDescription("MASS")  # get short description
        help(package="MASS")  # get full documentation for package "MASS"
        ls("package:MASS")  # list all functions in package "MASS"
        data(package="AER")  # show all datasets in package "AER"
