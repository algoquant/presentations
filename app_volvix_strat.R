...
cap_tion <- paste("VXX Regression Z-score Versus VTI Volatility and SVXY Prices")
...
ser_ver <- function(input, output) {
  ...
  vi_x <- reactive({
    ...
    vi_x <- na.omit(cbind(
      quantmod::Cl(get(sym_bol, rutils::etf_env)),
      quantmod::Cl(get("SVXY", rutils::etf_env))))
  ...
  pnl_s <- reactive({
    ...
    de_sign <- cbind(sqrt(vari_ance), svx_y)
  ...
  output$dy_graph <- dygraphs::renderDygraph({
    ...
    cap_tion <- paste("Strategy for", input$symbol_vix, "Regression Z-score / \n", 
