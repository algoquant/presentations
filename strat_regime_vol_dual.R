##############################
# This is a shiny app for simulating a dual volatility 
# regime switching strategy.
# 
# It uses the the difference between a fast and a slow
# range volatilities as a measure of volatility.
# 
...
captiont <- paste("Dual Volatility Regime Switching Strategy")
...
uifun <- shiny::fluidPage(
  ...
  fluidRow(
    ...
    column(width=2, sliderInput("lambdar", label="Returns decay", min=0.1, max=0.9, value=0.2, step=0.1)),
    ##  Input lambda variance decay parameter
    column(width=2, sliderInput("lambdas", label="Slow decay", min=0.1, max=0.9, value=0.9, step=0.1)),
    column(width=2, sliderInput("lambdaf", label="Fast decay", min=0.1, max=0.9, value=0.3, step=0.1)),
    ##  Input volatility scale parameter
    column(width=2, sliderInput("volscale", label="Scale", min=0.001, max=0.02, value=0.005, step=0.001)),
    ##  Input volatility threshold parameter
    column(width=2, sliderInput("volthresh", label="Vol threshold", min=0.001, max=0.05, value=0.005, step=0.001)),
  ),  ##  end fluidRow
...
servfun <- function(input, output) {
  ...
  retm <- shiny::reactive({
    ...
    rets <- rets()
    retp <- rets$daily
    retm <- HighFreq::run_mean(retp, lambda=lambdar)
  ...
  varv <- shiny::reactive({
    ...
    ##  Calculate the range variance
    ohlc <- ohlc()
    vars <- HighFreq::run_var_ohlc(ohlc, lambda=lambdas)
    varf <- HighFreq::run_var_ohlc(ohlc, lambda=lambdaf)
    # varv[1:2] <- 1
    return(sqrt(varf) - sqrt(vars))
