...
inter_face <- shiny::fluidPage(
  ...
  fluidRow(
    ...
    # Input weight
    column(width=3, sliderInput("wei_ght", label="weight",
                                min=(-1), max=5, value=1.0, step=0.1)),
    # Input look-back lag interval
    column(width=3, sliderInput("look_back1", label="Lookback short", min=3, max=20, value=5, step=1)),
    # Input look-back lag interval
    column(width=3, sliderInput("look_back2", label="Lookback long", min=10, max=50, value=25, step=1))
