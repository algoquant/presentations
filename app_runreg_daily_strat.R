...
cap_tion <- paste("Running Z-score of SVXY Prices Versus VXX app_runreg_daily_strat.R")
...
inter_face <- shiny::fluidPage(
  ...
  fluidRow(
    ...
    column(width=3, sliderInput("lamb_da", label="lamb_da:", min=0.01, max=0.99, value=0.4, step=0.01)),
    # Input threshold interval
    column(width=3, sliderInput("thresh_old1", label="Threshold1", min=(0.1), max=1.3, value=0.76, step=0.02)),
...
ser_ver <- function(input, output) {
  ...
  z_scores <- reactive({
    ...
    vari_ance <- HighFreq::run_var_ohlc(oh_lc[zoo::index(re_turns)], lambda=lamb_da)
    vari_ance[vari_ance == 0] <- 1
    vari_ance <- rutils::diff_it(vari_ance)
    predic_tor <- cbind(predic_tor, vari_ance)
    
    predictor_symbol <- input$predictor_symbol
    oh_lc <- log(get(predictor_symbol, rutils::etf_env))
    vari_ance <- HighFreq::run_var_ohlc(oh_lc[zoo::index(re_turns)], lambda=lamb_da)
    vari_ance[vari_ance == 0] <- 1
    vari_ance <- rutils::diff_it(vari_ance)
    predic_tor <- cbind(predic_tor, vari_ance)
    ...
    # response_symbol <- input$response_symbol
    # oh_lc <- log(get(response_symbol, rutils::etf_env))
    # vari_ance <- HighFreq::run_var_ohlc(oh_lc[zoo::index(re_turns)], lambda=lamb_da)
    # vari_ance[vari_ance == 0] <- 1
    # vari_ance <- rutils::diff_it(vari_ance)
    # predic_tor <- cbind(predic_tor, vari_ance)
