...
uiface <- shiny::fluidPage(
  ...
  fluidRow(
    ...
    column(width=3, sliderInput("lambdav", label="lambdav:", min=0.01, max=0.99, value=0.4, step=0.01)),
    # Input threshold interval
    column(width=3, sliderInput("threshold1", label="Threshold1", min=(0.1), max=1.3, value=0.76, step=0.02)),
...
servfunc <- function(input, output) {
  ...
  z_scores <- reactive({
    ...
    variance <- HighFreq::run_var_ohlc(ohlc[zoo::index(returns)], lambda=lambdav)
