...
servfunc <- function(input, output) {
  ...
  z_scores <- reactive({
    ...
    # Calculate the response and design
    returns <- returns()
    response <- returns[, 1]
    design <- returns[, -(1:2)]

    # Calculate the trailing z-scores
    # z_scores <- drop(HighFreq::roll_zscores(response=response, predictor=design, look_back=look_back))
    z_scores <- HighFreq::run_reg(response=response, predictor=design, lambda=lambdav, method="scale")
