...
uifun <- shiny::fluidPage(
  ...
  fluidRow(
    ...
    column(width=2, sliderInput("lambda", label="lambda:", min=0.1, max=0.99, value=0.5, step=0.01)),
...
servfun <- shiny::shinyServer(function(input, output) {
  ...
  betav <- shiny::reactive({
    ...
    covars <- HighFreq::run_covar(pricev(), lambda=input$lambda)
    # covars <- HighFreq::run_covar(retv(), lambda=input$lambda)
    covars[1, ] <- 1
    covars[, 1]/covars[, 3]
