...
uiface <- shiny::fluidPage(
  ...
  fluidRow(
    ...
    column(width=3, sliderInput("lambdav", label="lambdav:", min=0.01, max=0.99, value=0.81, step=0.01)),
    # Input threshold interval
    column(width=3, sliderInput("threshold1", label="Threshold1", min=(0.1), max=1.3, value=1.1, step=0.02)),
...
servfunc <- function(input, output) {
  ...
  returns <- reactive({
    ...
    # Load the data
    symbolv <- c(symbol, response_symbol, predictor_symbol)
