...
captiont <- paste("Dual Crossover Range Volatility Strategy")
...
servfun <- function(input, output) {
  ...
  pnls <- shiny::reactive({
    ...
    varf <- HighFreq::run_var_ohlc(ohlc=ohlc, lambdaf=lambdaf)
    vars <- HighFreq::run_var_ohlc(ohlc=ohlc, lambdaf=lambdas)

    # Determine dates when the EMAs have crossed
    indic <- -sign(varf - vars)
