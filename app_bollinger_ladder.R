...
uifun <- shiny::fluidPage(
  ...
  fluidRow(
    ...
    column(width=2, sliderInput("threshl", label="Lower threshold", min=-4.0, max=0.0, value=0.0, step=0.1)),
    # Input z-score upper threshold level
    column(width=2, sliderInput("threshu", label="Upper threshold", min=0.0, max=4.0, value=0.3, step=0.1)),
    # Input volatility threshold floor
    column(width=2, sliderInput("volf", label="volatility floor", min=0.0, max=0.15, value=0.0, step=0.01)),
...
servfun <- shiny::shinyServer(function(input, output) {
  ...
  wealthv <- shiny::reactive({
    ...
    # posv[1] <- 0
    # posv <- ifelse(pricez > threshz, coeff, posv)
    # posv <- ifelse(pricez < -threshz, -coeff, posv)
    ...
    for (td in 2:nrows) {
      if (pricez[td] > threshu) { # The z-score is greater than the threshold level
        ...
        if (posv[td-1] > 0) { # Unwind long position
          posv[td] <- coeff
        # The maximum z-score is larger than the position - add to short
        } else if (zscoremax[td] > -posv[td-1]) {
        ...
        if (posv[td-1] < 0) { # Unwind short position
          posv[td] <- -coeff
        # The minimum z-score is less than minus the position - add to long
        } else if (zscoremin[td] < -posv[td-1]) {
        # } else if ((pricezd[td] < 0) && (pricez[td-1] < -posv[td-1])) {
