...
# load OHLC data
returnv <- calc_rets(xts_data=to.daily(SPY))
...
colnames(daily_hurst) <- 
  paste(colnamev(get(symbol)), ".Hurst")
...
colnames(season_volume) <- 
  paste0(colnamev(get(symbol)), ".season_volume")
...
season_var <- season_ality(vol_ohlc(ohlc=SPY))
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.5\paperwidth,valign=t]{figure/daily_season_volume_volat.png}
  \end{columns}
\end{block}
...
colnames(season_illiquid) <- 
  paste0(colnamev(get(symbol)), ".season_illiquid")
...
season_var <- season_ality(vol_ohlc(ohlc=SPY))
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.5\paperwidth,valign=t]{figure/daily_season_illiquid_volat.png}
  \end{columns}
\end{block}
...
%%%%%%%%%%%%%%%
\subsection{Daily Seasonality of Hurst Exponent and Volatility}
\begin{frame}[fragile,t]{\subsecname}
\vspace{-1em}
\begin{block}{}
  \begin{columns}[T]
...
colnames(season_hurst) <- paste0(colnamev(get(symbol)), ".season_hurst")
plot_theme <- chart_theme()
plot_theme$format.labels <- "%H:%M"
...
season_var <- season_ality(vol_ohlc(ohlc=SPY))
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.5\paperwidth,valign=t]{figure/daily_season_hurst_volat.png}
  \end{columns}
\end{block}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      <<echo=(-(1:1))>>=
# skew scatterplot
returnv <- calc_rets(xts_data=SPY)
...
posit[1, ] <- 0
# cumulative PnL
cumu_pnl <- cumsum(posit*returnv[, 1])
...
interval <- "2013-12-01/"
plot(formulav, data=cbind(returnv[, 1], lag_vwap)[interval], 
     cex=0.6, xlab="skew", ylab="rets")
abline(regmod, col="blue", lwd=2)
      @
  \end{columns}
