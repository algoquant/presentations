...
captiont <- paste("Daily Ratchet Revert to EMA for", symbolpair)
...
uifun <- shiny::fluidPage(
  ...
  fluidRow(
    ...
    column(width=2, sliderInput("zfact", label="Z-factor", min=-1.0, max=1.0, value=1.0, step=0.1)),
    # Input the Z-score factor
...
servfun <- function(input, output) {
  ...
  pnls <- shiny::reactive({
    ...
    pnls <- lapply(pricel, function(pricev) {
      ...
      pospnls <- ratchets(pricev, lambdaf=input$lambdaf, zfact=input$zfact, poslimit=input$poslimit)
