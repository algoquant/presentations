...
uiface <- shiny::fluidPage(
  ...
  fluidRow(
    ...
    column(width=3, sliderInput("lambdav", label="lambdav:", min=0.01, max=0.9, value=0.73, step=0.01)),
...
servfunc <- function(input, output) {
  ...
  returns <- reactive({
    ...
    #   get(predictor1, rutils::etfenv$returns),
    #   get(predictor2, rutils::etfenv$returns)))
  ...
  z_scores <- reactive({
    ...
    z_scores <- HighFreq::run_zscores(response=response, predictor=predictor, lambda=lambdav)
  ...
  pnls <- reactive({
    ...
    cum_rets <- xts::xts(cumsum(rowSums(returns)), zoo::index(returns))
   .n_rows <- NROW(returns)
    ...
    variance <- HighFreq::run_var(tseries=HighFreq::diffit(z_scores[, 1, drop=FALSE]), lambda=lambdav)
    ...
    indic[1] <- 0
    indic[z_scores[, 1] > threshold] <- coeff
    indic[z_scores[, 1] < (-threshold)] <- (-coeff)
   ...
   .n_cols <- (NCOL(z_scores)-1)/2
    weights <- matrix(rep(NA_integer_,.n_cols.n_rows), ncol.n_cols)
    weights[1, ] <- 1
    betas <- z_scores[, 2:.n_cols+1)]
    se_lect <- (z_scores[, 1] > threshold)
    weights[se_lect, ] <- coeff*betas[se_lect, ]
    se_lect <- (z_scores[, 1] < (-threshold))
    weights[se_lect, ] <- -coeff*betas[se_lect, ]
    weights <- cbind(rep(1,.n_rows), weights)
    ...
    # response <- vx_x
    # z_scores <- drop(HighFreq::roll_zscores(response=response, design=predictor, look_back=look_back))
    # z_scores[1:look_back] <- 0
    # z_scores[is.infinite(z_scores)] <- 0
    # z_scores[is.na(z_scores)] <- 0
    # z_scores <- z_scores/sqrt(look_back)
    ...
    pnls <- pnls*sd(returns[returns[, 1]<0, 1])/sd(pnls[pnls<0])
    
    # Bind together strategy pnls
    pnls <- cbind(returns[, 1], pnls)
