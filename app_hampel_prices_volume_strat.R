...
inter_face <- shiny::fluidPage(
  ...
  fluidRow(
    ...
    column(width=2, sliderInput("thresh_old", label="threshold", min=0.5, max=3.0, value=0.9, step=0.1)),
...
ser_ver <- function(input, output) {
  ...
  z_scores <- reactive({
    ...
    cum_scaled <- cumsum(re_turns()/vol_ume)
    cum_scaled[is.na(cum_scaled) | is.infinite(cum_scaled)] <- 0
    ...
    z_scores <- (cum_scaled - mi_n)/(ma_x - mi_n)
