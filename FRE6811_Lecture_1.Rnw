...
\footnotesize{Students will be required to install and use the $R$ Interpreter and the $RStudio$ integrated development environment (\emph{IDE}) on their computers.  Students will be encouraged to bring their laptop computers to class and run the R Interpreter during the lecture.  Students will be required to become proficient with the \emph{RStudio IDE}.}
\end{block}
...
Quick-R website (related to \emph{R in Action} book)\\
...
\hskip1em\url{http://cran.r-project.org/web/packages/AER/index.html}\\
...
Install R Interpreter: \fullcite{website:cran}\\
Install: \fullcite{website:rstudioide}\\
Read chapters 1-3: \fullcite{website:rintro}\\
\end{block}
\vskip4ex
\pause

\begin{block}{Recommended}
Read: \fullcite{website:googlestyler}\\
Read chapters 1-3: \citetitle{rupperttexbook}\\
\end{block}
