...
daily_var <- apply.daily(x=get(symbol), 
                         FUN=moment_ohlc)
colnames(daily_var) <- 
...
formulav <- as.formula(paste(colnames(datav)[1], 
          paste(paste(colnames(datav)[-1], 
            collapse=" + "), "- 1"), sep="~"))
formulav
l_m <- lm(formulav, data=as.data.frame(datav))
summary(l_m)$coef
...
# vwap plot
vwap_short <- 
  vwapv(xtes=get(symbol), look_back=70)
vwap_long <- 
  vwapv(xtes=get(symbol), look_back=225)
...
# vwap scatterplot
returns <- calc_rets(xts_data=get(symbol))
vwap_short <- vwapv(xtes=get(symbol), look_back=70)
vwap_long <- vwapv(xtes=get(symbol), look_back=225)
vwap_diff <- vwap_short - vwap_long
...
dates <- "2013-12-01/"
plot(formulav, data=cbind(returns[, 1], lag_vwap)[dates], 
     cex=0.6, xlab="skew", ylab="rets")
abline(l_m, col="blue", lwd=2)
      @
  \end{columns}
