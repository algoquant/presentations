...
# vwap plot
vwap_short <- 
  vwapv(xtes=get(symbol), win_dow=70)
vwap_long <- 
  vwapv(xtes=get(symbol), win_dow=225)
...
# vwap scatterplot
returns <- calc_rets(xts_data=get(symbol))
vwap_short <- vwapv(xtes=get(symbol), win_dow=70)
vwap_long <- vwapv(xtes=get(symbol), win_dow=225)
vwap_diff <- vwap_short - vwap_long
