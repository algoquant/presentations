##############################
# This is a shiny app for simulating a trend following strategy 
# using the slope of the EMA volatility.
#
...
captiont <- paste("EMA Volatility Trend Strategy")
...
uifun <- shiny::fluidPage(
  ...
  fluidRow(
    ...
    # Input the trade lag
    column(width=2, sliderInput("lagg", label="lagg", min=1, max=51, value=1, step=1)),
...
servfun <- function(input, output) {
  ...
  ohlc <- shiny::reactive({
    ...
    return(get(symboln, rutils::etfenv))
  ...
  pnls <- shiny::reactive({
    ...
    ohlc <- ohlc()
    datev <- zoo::index(ohlc) # Dates vector
    nrows <- NROW(ohlc) # number of rows
    ...
    emaf <- sqrt(HighFreq::run_var_ohlc(ohlc, lambdaf=lambdaf))
    ...
    posv <- coeff*sign(rutils::diffit(emaf, lagg=lagg))
