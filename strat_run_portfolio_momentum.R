##############################
# This is a shiny app for simulating a running portfolio momentum 
...
# To run run_pca_momentum, compile this CPP file:
# if (!("run_pca_momentum" %in% ls())) {
#   cat("Compiling the C++ code for run_pca_momentum function.\n")
...
#   load(file="/Users/jerzy/Develop/lecture_slides/data/sp500_returns_overnight.RData")
# } # end if

load(file="/Users/jerzy/Develop/lecture_slides/data/sp500_returns_overnight_top.RData")
# retp <- reton["2000/", 1:150]
retp <- reton["2000/"]
cat("Stocks:\n", colnames(retp), "\n")
# retp <- reton["2000/", sample(ncol(reton), 100)]
# if (!("retstock100" %in% ls())) {
# cat("Loading the S&P500 returns data.\n")
...
symboln <- "SPY"
indeks <- get(symboln, rutils::etfenv$returns)
indeks <- indeks[datev]
...
uifun <- shiny::fluidPage(
  titlePanel("Running Portfolio Momentum Strategy"),
  ...
  fluidRow(
    ...
    column(width=2, sliderInput("dimax", label="Number of eigen vectors:", min=2, max=max(14, dimaxx), value=17, step=1)),
    column(width=3, sliderInput("lambdaf", label="Returns decay:", min=0.1, max=0.5, value=0.1, step=0.01)),
    column(width=3, sliderInput("lambdacov", label="Covariance decay:", min=0.7, max=0.99, value=0.73, step=0.01)),
    column(width=3, sliderInput("lambdaw", label="Weight decay:", min=0.9, max=0.99, value=0.99, step=0.01))
    # Scale returns
...
servfun <- function(input, output) {
  ...
  pnls <- shiny::reactive({
    cat("Recalculating the model with new parameters.\n")
    ...
    # Rerun the model
    pnls <- sim_portfoptim(retp, dimax, lambdaf, lambdacov, lambdaw)
    # To run run_pca_momentum, compile this file in R by running this command:
    # Rcpp::sourceCpp(file="/Users/jerzy/Develop/HighFreq/sandbox/test_fun.cpp")
