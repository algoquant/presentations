...
servfunc <- function(input, output) {
  ...
  ohlc <- reactive({
    ...
    switch(symbol,
           "SPY" = {
             ...
             value_s$closep <- log(quantmod::Cl(ohlc))
             value_s$returns <- rutils::diffit(value_s$closep)
             value_s$volumes <- Vo(ohlc)
             value_s.n_rows <- NROW(ohlc)
             #.n_rows <- NROW(ohlc)
             # log(Cl(ohlc))
  ...
  z_scores <- reactive({
    ...
    volumes <- short_back*value_s$volumes/HighFreq::roll_sum(value_s$volumes, look_back=short_back)
    # returns <- rutils::diffit(closep())
    # Calculate the cumulative returns scaled by the rolling volume
    cumsumv <- cumsum(value_s$returns/volumes)
  ...
  pnls <- reactive({
    ...
    pnls <- cumsum(position_s*value_s$returns)
    cumsumv <- cumsum(value_s$returns)
