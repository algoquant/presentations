...
# load OHLC data
re_turns <- calc_rets(x_ts=to.daily(SPY))
...
daily_var <- apply.daily(x=SPY, FUN=agg_regate, 
                        esti_mator="vol_ohlc")
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      <<intraday_season_volume_volat,echo=(-(1:2)),eval=FALSE,fig.width=7,fig.height=8,fig.show='hide'>>=
par(mfrow=c(2,1))  # set plot panels
library(HighFreq)  # load package "HighFreq"
...
colnames(season_volume) <- 
  paste0(na_me(get(sym_bol)), ".season_volume")
...
season_var <- season_var[-(nrow(season_var))]
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.5\paperwidth,valign=t]{figure/intraday_season_volume_volat.png}
  \end{columns}
\end{block}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      <<intraday_season_illiquid_volat,echo=(-(1:2)),eval=FALSE,fig.width=7,fig.height=8,fig.show='hide'>>=
par(mfrow=c(2,1))  # set plot panels
library(HighFreq)  # load package "HighFreq"
...
colnames(season_illiquid) <- 
  paste0(na_me(get(sym_bol)), ".season_illiquid")
ch_ob <- chart_Series(x=season_illiquid,
  name=paste(colnames(season_illiquid), "intraday seasonality"), 
  plot=FALSE)
y_lim <- ch_ob$get_ylim()
y_lim[[2]] <- structure(c(0.2, 0.6), fixed=TRUE)
ch_ob$set_ylim(y_lim)
plot(ch_ob)
# intraday seasonality of volatility
season_var <- season_ality(vol_ohlc(ohlc=SPY))
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.5\paperwidth,valign=t]{figure/intraday_season_illiquid_volat.png}
  \end{columns}
\end{block}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      <<intraday_season_hurst_volat,echo=(-(1:2)),eval=FALSE,fig.width=7,fig.height=8,fig.show='hide'>>=
par(mfrow=c(2,1))  # set plot panels
library(HighFreq)  # load package "HighFreq"
...
season_var <- season_ality(vol_ohlc(ohlc=SPY))
    ...
    \column{0.5\textwidth}
      \vspace{-1em}
      \includegraphics[width=0.5\paperwidth,valign=t]{figure/intraday_season_hurst_volat.png}
  \end{columns}
\end{block}
...
daily_var <- apply.daily(x=SPY, FUN=agg_regate, 
                        esti_mator="vol_ohlc")
colnames(daily_var) <- paste0(sym_bol, ".var")
daily_skew <- apply.daily(x=SPY, FUN=agg_regate, 
...
re_turns <- calc_rets(x_ts=SPY)
sk_ew <- skew_ohlc(ohlc=SPY[, -5])
colnames(sk_ew) <- paste0(sym_bol, ".skew")
lag_skew <- lag(sk_ew)
lag_skew[1, ] <- 0
da_ta <- cbind(re_turns[, 1], lag_skew)
...
abline(l_m, col="blue", lwd=2)
      @
    \column{0.5\textwidth}
      \includegraphics[width=0.5\paperwidth,valign=t]{figure/skew_scatter.png}
  \end{columns}
\end{block}
