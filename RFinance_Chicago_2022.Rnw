...
\begin{block}{}
  \begin{columns}[T]
    \column{0.5\textwidth}
      To avoid the problem of forecasting the returnv, we may choose to select a \emph{minimum variance} portfolio.
      \vskip1ex
...
arma::mat back_test(const arma::mat& excess, // Asset excess returns
                    const arma::mat& returnv, // Asset returns
...
\begin{block}{}
  % \begin{columns}[T]
    % \column{0.5\textwidth}
      ...
      \begin{lstlisting}[language=R,basicstyle=\tiny\ttfamily\bfseries,backgroundcolor=\color{anti_flashwhite},showstringspaces=FALSE]
arma::vec calc_weights(const arma::mat& returnv, // Asset returns
  ...
  case methodenum::maxsharpemed: {
    ...
    arma::vec colmeans = arma::trans(arma::median(returnv, 0));
  ...
  case methodenum::quantile: {
    ...
    weights = conv_to<vec>::from(arma::sum(arma::quantile(returnv, levels, 0), 0));
    break;
  }  // end quantile
  default : {
  ...
  case methodenum::voltarget: {
    ...
    weights = weights*vol_target/arma::stddev(returnv*weights);
    break;
  }  // end voltarget
  case methodenum::voleqw: {
    // Scale the weights to the volatility of the equal weight portfolio
    weights = weights*arma::stddev(arma::mean(returnv, 1))/arma::stddev(returnv*weights);
    break;
  }  // end voleqw
