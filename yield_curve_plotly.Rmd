...
rate_s <- na.omit(do.call(merge, as.list(env_rates)[c("DGS10", "DGS20")]))
rate_s <- data.frame(dates=index(rate_s), coredata(rate_s))
...
# p_lot <- plot_ly(data=rate_s, x=~dates, y=~DGS10, type="scatter", mode="lines", name="10yr Treasury rate")
...
yield_curves <- lapply(1:NCOL(rate_s), function (col_umn) {
  list(visible=FALSE,
       name=paste0('v=', col_names[col_umn]),
       x=row_names,
       y=rate_s[, col_umn])
})  # end lapply
...
# create slider list
slide_r <- lapply(1:NCOL(rate_s), function (col_umn) {
  ste_p <- list(args=list("visible", rep(FALSE, length(yield_curves))), method="restyle")
  ste_p$args[[2]][col_umn] <- TRUE
  ste_p
})  # end lapply

# plot all traces
p_lot <- plot_ly()
for (col_umn in 1:NCOL(rate_s)) {
  p_lot <- add_lines(p=p_lot, x=yield_curves[col_umn][[1]]$x, 
                     y=yield_curves[col_umn][[1]]$y, 
                     visible=yield_curves[col_umn][[1]]$visible, 
                     name=yield_curves[col_umn][[1]]$name, 
