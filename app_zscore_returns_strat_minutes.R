...
uifun <- shiny::fluidPage(
  ...
  fluidRow(
    ...
    column(width=2, sliderInput("lambda", label="lambda:", min=0.01, max=0.9, value=0.25, step=0.01)),
...
servfun <- function(input, output) {
  ...
  zscores <- shiny::reactive({
    ...
    # weights <- exp(-lambda*1:long_back)
    # weights <- weights/sum(weights)

    predv <- matrix(rep(1, NROW(retv())))
    # This need update
    zscores <- HighFreq::run_reg(respv=returns(), predictor=predv, lambda=lambda, method="standardize")
    ...
    # minv <- roll::rolregmodin(lowp, width=short_back)
    # maxv <- roll::rolregmodax(highp, width=short_back)
    # minv[1:short_back, ] <- 0
    # maxv[1:short_back, ] <- 1
