...
# Go long SPY if the Kelly ratio is below the negative threshold.
#
# The Kelly ratio is equal to the EMA return divided by the EMA 
# volatility.
#
# The Kelly ratios are calculated using the function HighFreq::run_var().
# 
...
uifun <- shiny::fluidPage(
  ...
  fluidRow(
    ...
    column(width=2, sliderInput("lambdaf", label="Lambda",
                                min=0.1, max=0.99, value=0.34, step=0.01)),
    # Input threshold for Kelly ratios
...
servfun <- function(input, output) {
  ...
  kellyr <- shiny::reactive({
    
    cat("Recalculating Kelly ratios for ", symboln, "\n")
    lambdaf <- input$lambdaf
    # Calculate the EMA returns and volatility.
    # volma <- HighFreq::run_var(hilo, lambda=lambdaf)
    # vixma <- volma[, 1]
    # volma <- sqrt(volma[, 2])
    # kellyr <- (hilo - vixma)/volma
    ...
    varm <- HighFreq::run_var(rets, lambdaf)
    # kellyr[1, ] <- 0.0
    kellyr <- varm[, 1]/varm[, 2]
    # kellyr <- kellyr/sd(kellyr)
    return(kellyr)
  ...
  pnls <- shiny::reactive({
    ...
    posv <- ifelse(kellyr > threshv, -1, posv)
    posv <- ifelse(kellyr < -threshv, 1, posv)
    ...
    pnlm <- HighFreq::run_mean(pnls, lambdaf)
    posv <- sign(rutils::lagit(pnlm, lagg=1))
    pnls <- -posv*rets
