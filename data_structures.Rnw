...
\begin{block}{}
  \begin{columns}[T]
    \column{0.3\textwidth}
      ...
      The function \verb|cat()| is useful in user-defined functions,\\
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.3\textwidth}
      ...
      Examples of $R$ \verb|types| are \verb|double|, \verb|character|, \verb|list|, etc.\\
      \vskip1ex
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.3\textwidth}
      The function \verb|str()| compactly displays the structure of an $R$ object,\\
      \vskip1ex
      The functions \verb|head()| and \verb|tail()| display the first and last rows of an $R$ object,\\
    \column{0.7\textwidth}
      \vspace{-1em}
      <<echo=6:30,tidy=FALSE,prompt=TRUE>>=
v.type <- c('rose', 'daisy', 'tulip')  # character vector
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.3\textwidth}
      ...
      \vskip1ex
      The function \verb|order()| returns a permutation vector,\\
      \vskip1ex
      \verb|order()| can take several vectors as input, to break any ties,\\
    \column{0.7\textwidth}
      \vspace{-1em}
      <<echo=6:30,tidy=FALSE,prompt=TRUE>>=
v.type <- c('rose', 'daisy', 'tulip')  # character vector
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.35\textwidth}
      The functions \verb|read.table()| and \verb|write.table()| read and write data frames from files,\\
      \vskip1ex
      The functions \verb|read.csv()| and \verb|write.csv()| are wrappers for \verb|read.table()| and \verb|write.table()|, and read and write data frames from CSV format files,\\
      \vskip1ex
      \verb|read.table()| and \verb|write.table()| are inefficient for very large data sets,\\
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.4\textwidth}
      ...
      Types of bad data:
      \begin{itemize}[]
        \item \verb|NA| means Not Available (missing) data,
        \item \verb|NaN| means Not a Number data,
        \item \verb|Inf| means numeric overflow - divide by zero,
      \end{itemize}
      \vskip1ex
      The functions \verb|is.na()| and \verb|is.nan()| test for NA and NaN values,\\
      \vskip1ex
      Many functions have a \verb|na.rm| parameter to remove NAs from input data,\\
    \column{0.6\textwidth}
      \vspace{-1em}
      <<tidy=FALSE,prompt=TRUE>>=
...
\begin{block}{}
  \verb|NULL| isn't bad data, but is a reserved word and represents a null object,\\
  \verb|NULL| is returned by functions whose value is undefined,\\
  Very often variables are initialized to \verb|NULL| before iteration,\\
\end{block}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.3\textwidth}
      The function \verb|complete.cases()| returns TRUE if a row has no NA values,\\
    \column{0.7\textwidth}
      \vspace{-1em}
      <<tidy=FALSE,prompt=TRUE>>=
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.3\textwidth}
      ...
      \vskip1ex
      The function \verb|na.locf()| replaces NAs with most recent non-NA prior to it,\\
      \vskip1ex
    \column{0.7\textwidth}
      \vspace{-1em}
      <<tidy=FALSE,prompt=TRUE>>=
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.35\textwidth}
      The functions \verb|read.csv()| and \verb|write.csv()| allow reading and writing matrices from CSV format files,\\
      \vskip1ex
      Using the \verb|*.csv()| functions is very inefficient for large data sets,\\
    \column{0.65\textwidth}
      \vspace{-1em}
...
\begin{block}{}
  \begin{columns}[T]
    \column{0.35\textwidth}
      ...
      \vskip1ex
      The function \verb|write.matrix()| is part of library \verb|MASS|,\\
      \vskip1ex
      The advantage of using the \verb|scan()| function is its speed, but it doesn't handle rownames,\\
